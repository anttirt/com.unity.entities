    ...
    class SystemQueriesInspector : PropertyInspector<SystemQueriesTab>
    {
        ...
        public override void Update()
        {
            ...
            foreach (var entry in m_QueryEntries)
            {
                ...
                UpdateQueryHeader(entry.QueryView, entry.QueryId, totalCount);

                if (entry.SeeAllButton != null)
                {
                    var capped = totalCount > entry.Data.MaxEntityDisplayCount;
                    entry.SeeAllButton.style.display = capped ? DisplayStyle.Flex : DisplayStyle.None;
                    if (capped)
                        entry.SeeAllButton.text = L10n.Tr("See all ...");
                }
            }
        }

        static void UpdateQueryHeader(QueryView queryView, int queryId, int entityCount)
        {
            var unit = entityCount == 1 ? k_Entity : k_Entities;
            queryView.HeaderName.text = $"{k_Query} #{queryId} ({entityCount} {unit})";
        }
