...
namespace Unity.Entities.Editor
{
    ...
    partial class UpdateHierarchySystem : SystemBase
    {
        ...
        /// <summary>
        /// Creates hierarchy nodes for newly detected entities and their children, grouped under
        /// the appropriate subscene, world, or parent node.
        /// </summary>
        unsafe void CreateNewNodes()
        {
...
#endif
            var parentSet = new NativeHashSet<Entity>(m_HierarchyEntityChanges.CreatedEntities.Length, Allocator.Temp);
            ...
            for (var i = 0; i < m_HierarchyEntityChanges.CreatedEntities.Length; i++)
            {
                ...
                if (SystemAPI.TryGetComponent(e, out Parent parentComp))
                {
                    ...
                    // Existing parent from a previous frame. Queue the parent so its child buffer is scanned for this new child.
                    else
                        parentSet.Add(parentComp.Value);
                }
                else
                {
                ...
                // Newly created parent entity. May already be queued above if a new child referenced it.
                if (IsParent(e))
                    parentSet.Add(e);
            }
            ...
            // Build the parents list from the set now that all parents have been collected
            var parents = new NativeList<Entity>(parentSet.Count, Allocator.TempJob);
            ...
            if (parents.Length > 0)
                CreateChildrenNodes(parents, showHiddenEntities);

            parents.Dispose();
        }

        /// <summary>
        /// For each parent entity, enumerates its children and emits hierarchy nodes grouped under the parent.
        /// Parents are sorted by depth so that ancestor nodes are created before their descendants.
        /// </summary>
        unsafe void CreateChildrenNodes(NativeList<Entity> parents, bool showHiddenEntities)
        {
            ...
            {
                if (EntityManager.HasBuffer<Child>(parents[i]))
                    estimatedChildCount += EntityManager.GetBuffer<Child>(parents[i]).Length;
            }

            // Pre-compute prefab types on main thread and count total children for container sizing.
            var childrenEntityCount = 0;
            var additionalLinkedEntityCount = 0;
            var prefabTypeLookup = new NativeHashMap<Entity, HierarchyPrefabType>(estimatedChildCount, Allocator.TempJob);
            for (var i = 0; i < parents.Length; i++)
            {
                ...
                // For prefabs, also pre-compute types for additional LinkedEntityGroup entities.
                if (EntityManager.HasComponent<Prefab>(parent) && EntityManager.HasBuffer<LinkedEntityGroup>(parent))
                {
                    var parentHasChildBuffer = EntityManager.HasBuffer<Child>(parent);
                    ...
                    for (var j = 0; j < linkedEntities.Length; j++)
                    {
                        ...
                        if (EntityManager.HasComponent<HideInHierarchy>(childEntity))
                            continue;
                        // Already counted via the Child buffer loop above.
                        if (EntityManager.HasComponent<Parent>(childEntity) && parentHasChildBuffer)
                            continue;
                        // PopulateParentBuffersJob emits from both Child and LinkedEntityGroup
                        // without deduplication, so the capacity must count both paths.
                        prefabTypeLookup[childEntity] = ComputePrefabTypeForEntity(childEntity);
            ...
            public void Execute(int index)
            {
                ...
                if (PrefabLookup.HasComponent(e) && LinkedEntityGroupLookup.HasBuffer(e))
                {
                    ...
                    for (var i = 0; i < linkedEntities.Length; i++)
                    {
                        ...
                        // Skip entities that have a Parent component when the parent has a Child buffer,
                        // since they will be added through that buffer instead.
