...
namespace Unity.Entities.Editor
{
    internal class HierarchyWorldHandler : HierarchyNodeTypeHandler, IHierarchyEditorNodeTypeHandler
    {
        ...
        protected override void Dispose(bool disposing)
        {
            ...
            // Destroy the hierarchy support systems when disposing the World handler.
            foreach (var world in World.s_AllWorlds)
            {
                var systemGroup = world.GetExistingSystemManaged<SimulationSystemGroup>();

                var subSceneCleanupSystem = world.GetExistingSystemManaged<SubSceneNodeCleanupSystem>();
                if (subSceneCleanupSystem != null)
                {
                    systemGroup?.RemoveSystemFromUpdateList(subSceneCleanupSystem);
                    world.DestroySystem(subSceneCleanupSystem.SystemHandle);
                }
        ...
        void UpdateRegisteredWorlds()
        {
            ...
            foreach (var world in World.s_AllWorlds)
            {
                ...
                var subSceneCleanupSystem = world.GetOrCreateSystemManaged<SubSceneNodeCleanupSystem>();
                systemGroup.AddSystemToUpdateList(subSceneCleanupSystem);
            }
        }
