...
namespace Unity.Entities.Editor
{
    internal static class SystemSearchProvider
    {
        ...
        static WorldFilter s_WorldFilter;
        ...
        internal static IEnumerable<SystemDescriptor> systems
        {
            get
            {
                ...
                if (SetupSystemDescriptors())
                    GetAllSystems();
                return (IEnumerable<SystemDescriptor>)s_Systems ?? System.Array.Empty<SystemDescriptor>();
            }
        }
        ...
        {
            Cleanup();
            s_WorldFilter = world == null ? WorldFilter.AllWorlds : WorldFilter.SingleWorld;
        ...
        static bool SetupSystemDescriptors()
        {
            ...
            switch (s_WorldFilter)
            {
                case WorldFilter.None:
                    Debug.LogWarning("System Search provider: cannot find a valid World");
                    return false;
                case WorldFilter.AllWorlds when World.All.Count == 0:
                    Debug.LogWarning("System Search provider: cannot find a valid World");
                    return false;
            }
            s_WorldProxyManager.SetUpdateAllWorlds(s_WorldFilter == WorldFilter.AllWorlds);
            s_WorldProxyManager.CreateWorldProxiesForAllWorlds();
            if (s_WorldFilter == WorldFilter.SingleWorld)
            {
                if (!s_WorldProxyManager.TryGetWorldProxy(s_CurrentWorld, out s_WorldProxy))
                    return false;
        ...
        {
            s_WorldFilter = WorldFilter.SingleWorld;
        ...
        static void Cleanup()
        {
            s_WorldFilter = WorldFilter.None;
            s_CurrentWorld = null;
            s_WorldProxy = default;
        ...
        static IEnumerable<SearchItem> FetchItems(SearchContext context, SearchProvider provider)
        {
            ...
            if (s_WorldFilter == WorldFilter.None)
            {
