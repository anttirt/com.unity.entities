...
{
    internal class HierarchyWorldHandlerTests
    {
        ...
        [Test]
        [TestCase(HierarchyWorldFilter.Live)]
        [TestCase(HierarchyWorldFilter.Conversion)]
        public void SettingUpWorldFilter_NonFilteredWorldsShows(HierarchyWorldFilter worldFilter)
        {
            ...
            try
            {
                HierarchyEntitiesSettings.SetTypesOfWorldsShown(worldFilter);
                ...
                for (var i = 0; i < tmpWorlds.Length; ++i)
                {
                    ...
                    var shouldBeVisible = (HierarchyWorldHandler.GetMainFlag(world) & (WorldFlags)worldFilter) != 0;
                    if (shouldBeVisible)
            ...
            finally
            {
                ...
                HierarchyEntitiesSettings.SetTypesOfWorldsShown((HierarchyWorldFilter)oldFlags);
            }
        }
        ...
        {
            var oldFlags = HierarchyEntitiesSettings.GetTypesOfWorldsShown();
            HierarchyEntitiesSettings.SetTypesOfWorldsShown(HierarchyWorldFilter.Live);
