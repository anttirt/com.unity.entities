...
namespace Unity.Entities.Editor.Tests
{
    ...
    class WorldProxyManagerTests
    {
        ...
        public void SetSelectedWorldProxy_WithFullPlayerLoopFalse_OnlyEnablesSelectedUpdater()
        {
            ...
            m_Manager.SetUpdateAllWorlds(false);
            m_Manager.CreateWorldProxiesForAllWorlds();
        ...
        public void SetSelectedWorldProxy_WithFullPlayerLoopTrue_EnablesAllUpdaters()
        {
            ...
            m_Manager.SetUpdateAllWorlds(true);
            m_Manager.CreateWorldProxiesForAllWorlds();
        ...
        [TestCase(WorldFlags.Streaming)]
        [TestCase(WorldFlags.Shadow)]
        [TestCase(WorldFlags.Staging)]
        [TestCase(WorldFlags.None)]
        public void CreateWorldProxy_SkipsNonUserFacingWorlds(WorldFlags flags)
        {
            var world = new World("NonUserFacingWorld", flags);

            m_Manager.CreateWorldProxiesForAllWorlds();

            Assert.That(m_Manager.TryGetWorldProxy(world, out _), Is.False, $"Worlds with {flags} flags should not create proxies");
